...
class KaloclipBot:
    ...
    async def _download_video(self, page, product):
        ...
        # ===== Pre-Phase1: จัดการ dialog/modal ที่ค้างอยู่ =====
        # กรณี 1: "leave without creating?" dialog — มี สร้าง{n} + ยกเลิก + ออก
        #   → คลิก สร้าง{n} เพื่อ start render จาก dialog นี้
        # กรณี 2: modal อื่นๆ → กด Escape
        self.log("  [Pre-Phase1] ตรวจ dialog/modal...")
        try:
            import re as _re2
            # ตรวจหา leave-without-creating dialog (สร้าง+เลข + ออก ปรากฏพร้อมกัน)
            dialog_btns = await page.evaluate("""
                () => Array.from(document.querySelectorAll('button'))
                    .filter(b => b.offsetParent !== null)
                    .map(b => b.textContent.trim())
            """)
            has_create = any(_re2.match(r'^สร้าง\s*\d+', t) for t in dialog_btns)
            ...
            if has_create and has_exit:
                ...
                await page.keyboard.press("Escape")
                await page.wait_for_timeout(800)
        except Exception as epre:
            self.log(f"  ⚠️ Pre-Phase1 dialog: {epre}")
    ...
    async def _post_to_tiktok(self, video_path: str, product_name: str) -> str:
        """Upload วิดีโอไปยัง TikTok แบบ Inbox Upload (video.upload scope)
        วิดีโอจะไปอยู่ใน Creator Inbox เพื่อ publish ต่อ
        ...
        try:
            # Step 1: Init Inbox Upload (video.upload scope — ไม่ต้องการ video.publish)
            # วิดีโอจะไปอยู่ใน Creator Inbox → user publish เอง
            ...
            self.log(f"✅ TikTok: อัปโหลดสำเร็จ! publish_id={publish_id} (อยู่ใน Creator Inbox — กด Publish จาก TikTok App)")
            return publish_id
